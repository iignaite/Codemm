...
**Phase 7 (bloat removal) is now substantially complete.** The legacy `perSlotGenerator`/`legacyAdapter` path (~2,000 lines, production-unreachable) is deleted; the eight unit tests that pinned it were replaced with staged-pipeline coverage (event ordering, stage retry, terminal failure, scaffolding decay, cpp starter synthesis), so the production path now has direct unit coverage it previously lacked. The session-named service API and its facade are retired (Thread names are primary, `services/threads` barrel), the single-plugin runtime registry is collapsed into `runtimeService`, and the dead `/chat` route, splash back-compat shapes, and seven orphaned modules (`utils/javaSource`, `utils/javaRewrite`, `agent/{dialogueAct,deferDefaults,ambiguity}`, `languages/java/{sampleDrivenTests,structuralTopics}`, `infra/llm/routePlanner`, `utils/traceBus`) are removed — roughly 4,800 lines deleted against ~400 added, with all suites green.

**Robustness batch (2026-07-09, second tranche).** Persistence failures in run logging and progress fan-out now log structured warnings instead of vanishing in bare `catch {}` (a shared `bestEffort` helper). Migrations are a versioned declarative table stamped via `PRAGMA user_version` (idempotent baseline for pre-versioning databases; future migrations run once), with tests for legacy sessions-era databases, fresh schema, and re-runs. The IPC boundary has handler-level tests for schemas, size limits, and error paths across judge/threads/activities/learning. Local models gained three guards: a `num_ctx` cap on every completion (default 8192, `CODEMM_OLLAMA_NUM_CTX` to override), memory-availability-aware model selection (`process.availableMemory`-based, demotes under genuine starvation, never hard-blocks), and a structured-output micro-eval after the readiness probe — a model that cannot emit one exact JSON object is routed as `weak` regardless of size, recorded in `LocalLlmStatus.runtime.capabilities`.

**Cleanup + resilience (2026-07-09, third tranche).** Removed misleading/duplicated non-code assets (53 self-deprecated doc stubs, legacy `documentation/` trees, duplicate backend screenshot set) and the review page's copy-local-link UI (a boot-token-guarded `127.0.0.1` URL is dead on restart). The generation soft-fallback is now a multi-rung degradation ladder — hard→medium, then topic narrowing, each rung firing once until exhausted — and terminal failures tell the learner how many problems were kept and that Generate resumes from the first missing slot.

**Learning path shipped (2026-07-09, fourth tranche).** All three open product decisions were resolved (free-Gemini as a labeled option; Docker-optional browse mode; tag-keyed concepts) and implemented: a free-Gemini onboarding tier with modernized models; a non-fatal Docker boot gate so the app launches for browsing without Docker (judge handlers refuse with a clear message); and Phases 3 + 9 — a deterministic `buildLearningPath` engine over persisted mastery, `learning.getPath` IPC, and a `/roadmap` skill-map UI. Codemm now *shows* the learner their progress, next step, and per-concept skill gaps — the mastery data is no longer invisible.

**Simplification pass (2026-07-09, fifth tranche).** All frontend eslint errors cleared (typed Monaco refs, typed bridge accessors, `useSyncExternalStore`-based theme hook that also fixes cross-page theme sync). Judge run/submit share a declarative `FILE_RULES` table + one `validateUserFiles` helper instead of ~70 duplicated lines; error messages pinned by the IPC boundary tests. The `ExecutionEngine` framework (Phase 8) is gone: both call sites were fixed linear sequences using none of its features, and are now a plain loop (orchestrator) and three inline phases (`generateFromThread`), verified by the staged-pipeline unit tests and the e2e suite including the failure path.

Still open (in priority order): non-root judge containers + `--cap-drop` (needs verification on a machine with Docker; interacts with host-mount permissions), roadmap iteration (deep-link "work on next" into generation; surface goal concepts not yet started), and the remaining Phase 8 item (`ExecutionEngine` inlining — deliberately deferred: it is live infrastructure in the generation path and inlining is a clarity-only change). One product/pedagogy question surfaced by the cleanup: `structuralTopics` enforced that hard Java topics appear structurally in reference solutions, but only the legacy generator ever called it — decide whether the staged pipeline should enforce it (stricter acceptance) before reintroducing from git history.
