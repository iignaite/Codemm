...
import { inferModelCapability } from "./capability";
...
function normalizeRoute(provider: ResolvedLlmRoutePlan["provider"], route: LlmRoute | undefined, defaultModel?: string): LlmRoute {
  ...
  return {
    ...(model ? { model } : {}),
    capability: route?.capability ?? inferModelCapability(model, provider),
...
export function getRouteForRole(
  ...
  return {
    ...
    capability: inferModelCapability(escalatedModel, normalized.provider),
  };
}
