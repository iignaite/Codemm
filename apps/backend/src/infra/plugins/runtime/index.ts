...
function normalizeRoute(provider: ResolvedLlmRoutePlan["provider"], route: LlmRoute | undefined, defaultModel?: string): LlmRoute {
  ...
  return {
    ...(model ? { model } : {}),
    capability: route?.capability ?? inferCapability(model, provider),
    ...(fallbackChain.length > 0 ? { fallbackChain } : {}),
    ...(route?.promptTemplateId ? { promptTemplateId: route.promptTemplateId } : {}),
  };
}

const defaultRuntimePlugin: RuntimePlugin = {
  id: "route-plan-runtime",
  ...
  summarizeRoutePlan(plan) {
    const normalized = defaultRuntimePlugin.normalizeRoutePlan(plan);
    if (!normalized) return null;
    const modelsByRole: Record<string, unknown> = {};
    for (const role of ROLE_ORDER) {
      const route = defaultRuntimePlugin.getRouteForRole(normalized, role);
  ...
  inferCapability,
  getRouteForRole(plan, role, opts) {
    const normalized = defaultRuntimePlugin.normalizeRoutePlan(plan);
    if (!normalized) return null;
    ...
    return {
      ...
      capability: inferCapability(escalatedModel, normalized.provider),
    };
  },
};

const runtimePlugins: RuntimePlugin[] = [defaultRuntimePlugin];

export function listRuntimePlugins(): RuntimePlugin[] {
  return [...runtimePlugins];
}

export function getDefaultRuntimePlugin(): RuntimePlugin {
  return runtimePlugins[0]!;
}
